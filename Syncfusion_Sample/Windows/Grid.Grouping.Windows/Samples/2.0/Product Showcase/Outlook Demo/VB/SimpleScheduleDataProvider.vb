...
'INSTANT VB NOTE: The variable masterList was renamed since Visual Basic does not allow class members with the same name:
		...
		Public Property MasterList() As SimpleScheduleItemList
			...
			End Get
			Set(ByVal value As SimpleScheduleItemList)
		...
		Public Shared Function InitializeRandomData() As SimpleScheduleItemList
			...
			Dim masterList As New SimpleScheduleItemList()
...
#End If
		...
		Public Overrides Overloads Function GetSchedule(ByVal startDate As DateTime, ByVal endDate As DateTime) As IScheduleAppointmentList
		...
		''' <returns>Returns a the subset of MasterList.</returns>
		Public Overrides Overloads Function GetScheduleForDay(ByVal day As DateTime) As IScheduleAppointmentList
			Dim list As New ScheduleAppointmentList()
			day = day.Date
			For Each item As ScheduleAppointment In Me.MasterList
		...
		Public Sub SaveXML(ByVal fileName As String)
			...
			Dim serializer As New XmlSerializer(a.GetType()) 'typeof(SimpleScheduleItem));
		...
		Public Sub SaveBinary(ByVal fileName As String)
			Dim s As Stream = File.Open(fileName, FileMode.OpenOrCreate)
			SaveBinary(s)
		End Sub
		...
		Public Shared Function LoadXML(ByVal fileName As String) As SimpleScheduleDataProvider
			Dim t As New SimpleScheduleDataProvider()
			Dim s As Stream = File.OpenRead(fileName)

			Try
				Dim serializer As New XmlSerializer(GetType(SimpleScheduleItem())) 'typeof(SimpleScheduleItem));
				Dim a() As SimpleScheduleItem = TryCast(serializer.Deserialize(s), SimpleScheduleItem())
		...
		Public Shared Function LoadBinary(ByVal fileName As String) As SimpleScheduleDataProvider
			...
			Try
				...
				t.MasterList = TryCast(obj, SimpleScheduleItemList)
			Finally
				s.Close()
	...
	''' </summary>
    Public Class SimpleScheduleItemList
...
#Region "ISerializable Members"
        ...
        ''' <param name="context">The StreamingContext.</param>
        Protected Sub New(ByVal info As SerializationInfo, ByVal context As StreamingContext)
            Me.UniqueID = CInt(Fix(info.GetValue("UniqueID", GetType(Integer))))
            Me.Subject = CStr(info.GetValue("Subject", GetType(String)))
            Me.StartTime = CDate(info.GetValue("StartTime", GetType(DateTime)))
            Me.ReminderValue = CInt(Fix(info.GetValue("ReminderValue", GetType(Integer))))
            Me.Reminder = CBool(info.GetValue("Reminder", GetType(Boolean)))
            Me.Owner = CInt(Fix(info.GetValue("Owner", GetType(Integer))))
            Me.MarkerValue = CInt(Fix(info.GetValue("MarkerValue", GetType(Integer))))
            Me.LocationValue = CStr(info.GetValue("LocationValue", GetType(String)))
            Me.LabelValue = CInt(Fix(info.GetValue("LabelValue", GetType(Integer))))
