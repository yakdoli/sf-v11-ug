...
namespace ExcelToolTip
{
	...
	public class CommentMouseController : IMouseController
	{
		...
		public CommentMouseController(GridControlBase owner, Hashtable comments)
		{
        ...
        private void IntializeControl(GridControlBase owner)
        {
            this.owner = owner;
            ...
            owner.CellMouseHover += new GridCellMouseEventHandler(owner_CellMouseHover);
            owner.CellMouseHoverLeave += new GridCellMouseEventHandler(owner_CellMouseHoverLeave);
        ...
        void owner_CellMouseHoverLeave(object sender, GridCellMouseEventArgs e)
        {
            MouseHoverLeave(EventArgs.Empty);
        }

        void owner_CellMouseHover(object sender, GridCellMouseEventArgs e)
        {
		...
		private void grid_ContextMouseUp(object sender, MouseEventArgs e)
		{
			if(e.Button == MouseButtons.Right || leftClickOnMark)
			{
				...
				if(this.owner.PointToRowCol(pt, out rightClickRow, out rightClickCol))
				{
					...
					if(isExcelTipCell(rightClickRow, rightClickCol))
					{
                    ...
                    string key=GetKey(rightClickRow, rightClickCol);
                    if (key!=null)
                    {
        ...
        {
            GridTableCellStyleInfo sty = this.owner.Model[row, col] as GridTableCellStyleInfo;
            if (sty.TableCellIdentity.DisplayElement.Kind == Syncfusion.Grouping.DisplayElementKind.Record)
            {
                if(sty.TableCellIdentity.Column!=null)
                    return sty.TableCellIdentity.DisplayElement.GetRecord().Id + sty.TableCellIdentity.Column.Name;
            }
            return null;
        }
		...
		{			
			...
			Rectangle rect = this.owner.RangeInfoToRectangle(GridRangeInfo.Cell(rightClickRow, rightClickCol)); 

			Point pt = new Point(rect.X, rect.Y);

			pt.X += this.owner.Model.ColWidths[rightClickCol]; 

            if (this.owner.ClientRectangle.Width > pt.X)
                this.commentWindow.InitializeComment(rightClickRow, rightClickCol, pt, text);
            else
            {
                pt.X = pt.X - 100;
                this.commentWindow.InitializeComment(rightClickRow, rightClickCol, pt, text);
		...
		{
            if (lastHitTestCode == GridHitTestContext.None)
                return;        
			...
			if(!this.commentWindow.Visible)// && !inDraw)
			{
				...
				owner.PointToRowCol(pt, out rowIndex, out colIndex, -1);
                    ...
                    string key =GetKey(rowIndex, colIndex);
                    string text = comments[key].ToString();
                    this.commentWindow.InitializeComment(rowIndex, colIndex, pt, text);
                    ...
                    if (this.owner.ClientRectangle.Width < ploc.X)
                        ploc.X = ploc.X - 100;
		...
		public int HitTest(MouseEventArgs e, IMouseController controller)
		{
			...
			{             
                if (isExcelTipCell(rowIndex, colIndex))
                {
		...
		private bool isExcelTipCell(int rowIndex, int colIndex)
		{
            string key = GetKey(rowIndex, colIndex);
            if (key == null)
                return false;
		...
		{
            if (isExcelTipCell(e.RowIndex, e.ColIndex) && (!this.owner.CurrentCell.HasCurrentCellAt(e.RowIndex, e.ColIndex) 
		...
		private void textBox_Leave(object sender, EventArgs e)
		{
			...
			{               
                string key = GetKey(commentRow, commentCol);
                comments[key] = this.commentWindow.editTextBox.Text;
