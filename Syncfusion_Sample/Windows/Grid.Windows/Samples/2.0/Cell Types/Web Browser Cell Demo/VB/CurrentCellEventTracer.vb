...
Namespace GridSample
	...
	Public Class CurrentCellEventTracer
		...
		Public Sub WireGrid(ByVal grid As GridControl)
			...
			AddHandler grid.QueryOleDataSourceData, AddressOf grid_QueryOleDataSourceData
		...
		Public Sub UnwireGrid(ByVal grid As GridControl)
			...
			RemoveHandler grid.QueryOleDataSourceData, AddressOf grid_QueryOleDataSourceData
			...
			If grid IsNot Nothing Then
				If grid.CurrentCell.HasCurrentCell Then
                    ...
                    Else
                        a = ""
                    End If
                    s = String.Format("{0}{1} {2}""{3}"", ""{4}"", StyleText: ""{5}"", StyleValue: ""{6}"" ", a, name, grid.CurrentCell.RangeInfo, ct, cv, st, sv)
				Else
					s = name
				End If
			End If
		...
		Private Sub AppendOutput(ByVal name As String, ByVal e As EventArgs)
			...
			If s.Length > 0 Then
				If s <> lastLine Then
					...
					_output.Items.Add(s & Constants.vbLf)
