...
namespace ExcelLikeUI_2005
{
	...
	public class ExcelMarkerMouseController : IMouseController
	{
		...
		public void MouseUp(MouseEventArgs e)
		{
			...
			{
				int top = this.grid.Model.Selections.Ranges.ActiveRange.Top;
				int bottom = this.grid.Model.Selections.Ranges.ActiveRange.Bottom;
				int left = this.grid.Model.Selections.Ranges.ActiveRange.Left;
				int right = this.grid.Model.Selections.Ranges.ActiveRange.Right;
				int row = 0;
				int col = 0;
				bool flag = false;

				if(this.grid.Model.Selections.Ranges.Count > 0)
					flag = true;

				cc = this.grid.CurrentCell;
				Point p = this.grid.PointToClient(Control.MousePosition);
				this.grid.PointToRowCol(p, out row, out col);
				if(flag)
				{
					if(row > bottom || row < top || col < left || col > right)
					{
						ActiveXSnapshot.FakeLeftMouseClick(this.grid, p);
					}
					flag = false;
				}
				this.grid.CurrentCell.MoveTo(row,col,GridSetCurrentCellOptions.ScrollInView);
			}
