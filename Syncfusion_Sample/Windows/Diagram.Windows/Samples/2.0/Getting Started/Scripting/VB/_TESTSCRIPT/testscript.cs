			...
			foreach (Node n in evt.Elements)
			{
				if (n is FilledPath)
				{
                    ...
                    if (evt.ChangeType == Syncfusion.Windows.Forms.Diagram.CollectionExChangeType.Insert)
                    {
                        ...
                        MessageBoxAdv.Show.Show(String.Concat(n.Name, " node added"), "Essential Suite Scripting", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                    else if (evt.ChangeType == Syncfusion.Windows.Forms.Diagram.CollectionExChangeType.Remove)
                    {
                        MessageBoxAdv.Office2007Theme = Office2007Theme.Blue;
                        MessageBoxAdv.Show(String.Concat(n.Name, " node Removed"), "Essential Suite Scripting", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
